...
if (!proof || !apiBase) { console.error('Faltan N8N_API_KEY / N8N_API_URL|N8N_BASE_URL en .env.local'); process.exit(2) }

async function main() {
  let res
  try {
    res = await fetch(`${apiBase}/workflows/${WF_ID}`, { headers: { 'X-N8N-API-KEY': proof, accept: 'application/json' } })
  } catch (e) {
