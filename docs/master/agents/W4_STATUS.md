...
- W2 head `43ce9b5` remains transport-neutral documentation/contracts. Route IDs now reject path/query/control syntax and 36/36 official contract tests pass. Its new telemetry allowlist is useful, but W4 reproduced a mutable-envelope TOCTOU and unbounded `contractVersion`; PR `#8` may continue while W2 fixes this P1.
...
| P1 | W2 telemetry returns the validated input object by reference and accepts arbitrary string/number `contractVersion` values | Post-validation mutation or version-field abuse can bypass the closed telemetry boundary | Frontend observability | Return a detached exact projection and close/bound the version value | Mutation and oversized/secret-shaped/non-finite negative tests pass |
...
- **W2:** draft PR `#8` at `43ce9b5` remains transport-neutral. Route IDs are fixed and all 36 official tests pass. Two disposable W4 telemetry attacks fail because accepted envelopes alias mutable caller input and `contractVersion` is unbounded. W4 posted a non-blocking P1 review; runtime still waits for an accepted W1 base and trusted sink integration.
