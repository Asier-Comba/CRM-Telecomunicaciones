...
- W3 head `190a615` fixes the original framework-core confirmation and atomic-reservation exploits and CI run `#56` is green. Draft PR `#9` remains blocked by W4 `CHANGES_REQUESTED` because durable stores, crash/reconciliation behavior and source-value redaction still do not satisfy Issue `#10`.
