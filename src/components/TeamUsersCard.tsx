...
//   - Sólo se renderiza si la membresía activa es owner o admin.
//   - Un admin solo puede asignar member/viewer; owner gestiona todos los roles.
            ...
            <tbody className="divide-y divide-gray-100">
              {users.map((u) => (
                <UserRow
                  ...
                  canDelete={isOwner}
                  canEditRole={u.role !== 'owner' || isOwner}
...
function UserRow({
  ...
  const allowedRoles: string[] = showOwnerOptions
    ? ['owner', 'admin', 'member', 'viewer']
    : ['member', 'viewer']

  // No exponemos en el selector roles a los que el usuario no podría llegar.
  const visibleRoles = user.role === 'owner' && !showOwnerOptions
    ? [user.role]
