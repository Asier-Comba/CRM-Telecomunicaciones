...
const injectionMarker = ['fixture', 'turn', 'policy', 'marker'].join('-')
const NOW = 1_700_000_000_000

function tokenFor(q: string, nowMs = NOW): string {
  const d = decideTurn(q)
  return signTurnPolicy({ cid: 'c', tid: 't', domain: d.domain, read: d.shouldReadData, write: d.shouldWriteData, tools: allowedToolsForTurn(d) }, injectionMarker, nowMs)
}
...
export function runTurnPolicyStrictEvals(): string[] {
  ...
  ok(ev({ tool: 'search_clients', token: readTok, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW }).action === 'allow', 'A: token válido + permitida → allow')
  // B) Token válido + tool no permitida (cross-domain) → reject 403 tool_not_allowed_for_turn.
  {
    const r = ev({ tool: 'search_properties', token: readTok, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW })
  ...
  // C) Turno no-datos (meta) → cualquier read rechazada.
  {
    ...
    const r = ev({ tool: 'search_clients', token: metaTok, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW })
  ...
  {
    const r = ev({ tool: 'search_clients', token: readTok, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW + 10_000_000 })
  ...
  // E) Token manipulado → reject invalid_turn_policy.
  {
    ...
    const r = ev({ tool: 'search_clients', token: tampered, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW })
  ...
  {
    const r = ev({ tool: 'search_clients', token: null, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: false, nowMs: NOW })
  ...
  {
    const r = ev({ tool: 'search_clients', token: null, secret: injectionMarker, requirePolicy: false, allowUnscopedDev: false, nowMs: NOW })
    ok(r.action === 'warn_unscoped', 'F2: sin token + compat → warn_unscoped')
  }
  // F3) Sin token + strict + allowUnscopedDev → warn (solo dev).
  {
    const r = ev({ tool: 'search_clients', token: null, secret: injectionMarker, requirePolicy: true, allowUnscopedDev: true, nowMs: NOW })
  ...
  for (const [tok, rp] of [[readTok, true], [null, false]] as [string | null, boolean][]) {
    const r = ev({ tool: 'get_invoices_summary', token: tok, secret: injectionMarker, requirePolicy: rp, allowUnscopedDev: false, nowMs: NOW })
