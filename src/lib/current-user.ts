...
export type ProfileRole = WorkspaceRole
...
export type CurrentUser = {
  ...
  isFallback: boolean
  trialLabel: string
  // Display-only role from the selected active workspace membership. Server
  // routes must resolve membership again and never trust this client value.
  role: ProfileRole
...
export function buildCurrentUser(context: ResolvedWorkspaceContext): CurrentUser {
  ...
  const role: ProfileRole = context.workspaceRole ?? 'viewer'

  return {
    id: user.id,
    name,
    email,
    workspaceId: context.resolvedWorkspaceId || undefined,
    workspaceName,
    ...
    isFallback: false,
    trialLabel: workspace?.status === 'suspended' ? 'Workspace suspendido' : 'Cuenta activa',
    role,
