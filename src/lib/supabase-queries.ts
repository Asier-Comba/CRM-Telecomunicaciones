...
export async function listWorkspaceProfiles(workspaceId: string): Promise<WorkspaceMember[]> {
  ...
  const { data: memberships, error: membershipError } = await supabase
    ...
    .eq('status', 'active')
  if (membershipError) {
    if (process.env.NODE_ENV === 'development') console.warn('[listWorkspaceProfiles]', membershipError.message)
    return []
  }
  const memberRows = (memberships as DataRecord[] | null) ?? []
  const userIds = memberRows.map((row) => asString(row.user_id)).filter(Boolean)
  ...
  const roles = new Map(memberRows.map((row) => [asString(row.user_id), asString(row.role)]))
  return (((profiles as DataRecord[] | null) ?? [])).map((profile) => {
    const fullName = asString(profile.full_name)
    const email = asString(profile.email) || null
    return {
      id: asString(profile.id),
      name: fullName || (email ? email.split('@')[0] : '') || 'Miembro',
      email,
      role: roles.get(asString(profile.id)) || null,
    }
  })
}
...
async function resolveWorkspaceIdentityUncached(): Promise<ResolvedWorkspaceContext> {
  ...
  if (membershipError) {
    return {
      ...
      workspaceId: null,
      resolvedWorkspaceId: null,
      error: 'Active workspace membership could not be resolved',
      profileLookupMethod: profileResult.profileLookupMethod,
      profileByIdError: profileResult.profileByIdError ?? null,
      profileByEmailError: profileResult.profileByEmailError ?? null,
    }
  ...
  const memberships: ActiveMembership[] = normalizeActiveMemberships(membershipRows ?? [])
  const selection = selectActiveMembership(memberships, null, profile.workspace_id ?? null)
